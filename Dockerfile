...
LABEL description="An LLM evaluation tool that uses a model-to-model qualitative comparison metric."
...
ENV DJANGO_SETTINGS_MODULE=parlance.settings.container
RUN mkdir staticfiles
RUN python3 manage.py collectstatic

CMD [ "gunicorn", "--bind", "0.0.0.0:8000", "parlance.wsgi", "--log-file", "-", "--capture-output" ]